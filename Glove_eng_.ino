...
void loop()
{
  ...
  thumb   = map(thumb  ,ClosedThumb  ,OpenedThumb  ,80,0);  // The analog read has to be readapted in values between 0 and 180 to be used by the servomotors.
  index   = map(index  ,ClosedIndex  ,OpenedIndex  ,80,0);  // The minimum and maximum values from the calibrations are used to correctly set the analog reads.
  middle  = map(middle ,ClosedMiddle ,OpenedMiddle ,80,0);
  annular = map(annular,ClosedAnnular,OpenedAnnular,80,0);  
  pinky   = map(pinky  ,ClosedPinky  ,OpenedPinky  ,80,0);
